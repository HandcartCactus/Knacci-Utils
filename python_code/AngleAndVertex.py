...
def vertices(w:str, alpha:float, a0:float=pi/2):
    ...
    for k,w_k in enumerate(w):
        ...
        #addvec the step to the current position
        #we start having taken the first step 
        #(drawing angle is changed *after* step is taken, so say the papers.)
        if k!=len(w)-1:    
            pos = addvec(pos, unit_vec(coeff * alpha + a0))
    
    return x,y

def draw_me(w:str, alpha:float, a0:float=pi/2):
    x, y = vertices(w, alpha, a0)
    plt.plot(x, y)
    plt.show()
...
def f2n_analysis(max_n=18):
    ...
    for n in range(1,18,1):
        ...
        print("n={},\tfirstfew={:5s},  <x,y> = <{:=0.2f}, {:=0.2f}>,\tvecang={:0.3f} pi,\tendcoeff={} alpha,\tL_n={:0.3f},\tB={:0.7f},\tB'={:0.7f}".format(n,f2n[0:5],pos[0],pos[1], acos(get_uvec(pos)[1])/pi, coeff, absvec(pos), absvec(pos)/absprev2, absvec(pos)/absprev1))
        absprev2 = absprev1
        absprev1 = absvec(pos)
